...
/**
 * A saved-spot card. On phones the actions live in a row under the content
 * (always tappable, never covering the thumbnail); from md: up they overlay
 * on hover like before.
 */
export default function SpotRow(props: SpotRowProps) {
  ...
  return (
    <div
      ...
      className="group relative flex flex-col p-3 md:p-4 rounded-card glass hover:bg-white/[0.06] transition-all cursor-pointer overflow-hidden"
    >
      <div className="flex items-center gap-3 md:gap-5">
        ...
        <div className="w-14 h-14 md:w-20 md:h-20 rounded-2xl md:rounded-card overflow-hidden bg-black border border-white/5 flex-shrink-0 relative group-hover:scale-105 transition-transform duration-500">
          {spot.thumbnail ? (
      ...
      {/* Mobile: action row under the content */}
      <div className="flex md:hidden items-center justify-end gap-1.5 mt-2 pt-2 border-t border-white/5">
        <ActionButtons {...props} />
      </div>

      {/* Desktop: hover overlay */}
      <div className="hidden md:flex absolute right-2 top-2 flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <ActionButtons {...props} />
      </div>
    </div>
  );
